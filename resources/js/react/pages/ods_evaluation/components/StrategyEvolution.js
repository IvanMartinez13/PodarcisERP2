...
class StrategyEvolution extends React.Component {
    ...
    render() {
        ...
        return (
            <canvas
                id={"strategy_evolution" + this.strategy.token}
    ...
    componentDidMount() {
        axios
            .post("/ods/strategy/evolution_chart", {
            ...
            .then((response) => {
                ...
                if (typeof this.targetValue == "number") {
                    $("#strategy_target_value" + this.strategy.token).text(
                        this.formatValue(targetValue.toFixed(3)) +
                            " " +
                            this.strategy.indicator
                    );
                } else {
                    $("#strategy_target_value" + this.strategy.token).text(
                        "Introduce un valor en el año base"
                    );
                }
            ...
            .then(() => {
                if (typeof this.targetValue == "number") {
                    let ctx = document
                        .getElementById(
                            "strategy_evolution" + this.strategy.token
                        )
                        .getContext("2d");
